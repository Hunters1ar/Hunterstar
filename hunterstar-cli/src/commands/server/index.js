...
export async function runServerCommand(args) {
    ...
    let isRestart = args.includes('--restart');
    ...
    if (isOn && !isDaemon && sessionManager.getSession()) {
        isRestart = true;
    }
    ...
    if (isDaemon || isForeground) {
        ...
        if (analysis.backend) {
            ...
            startupPromises.push((async () => {
                ...
                const { proc } = await pm.startProcess('BACKEND', analysis.backend.command, analysis.backend.cwd, '\x1b[32m', backendEnv);
        ...
        if (analysis.frontend) {
            ...
            startupPromises.push((async () => {
                const { proc } = await pm.startProcess('FRONTEND', analysis.frontend.command, analysis.frontend.cwd, '\x1b[36m', { PORT: process.env.FRONTEND_PORT || 3000 });
